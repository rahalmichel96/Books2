...
class Favoris_ActivityActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        books = ArrayList()
        var resultatJson = intent.getStringExtra("results")

        //val booksAdapter : BooksAdapter = BooksAdapter(books, { book -> bookOnclick() })

        recyclerView = findViewById(R.id.recycler_view)
        recyclerView!!.layoutManager  = LinearLayoutManager(this@Favoris_ActivityActivity)
        recyclerView!!.adapter = BooksAdapter(books.toTypedArray(), {book -> bookOnclick()}, this@Favoris_ActivityActivity)
    ...
    fun getResults(page : Int, limit : Int) {
        ...
        val queue = Volley.newRequestQueue(this@Favoris_ActivityActivity)
        ...
        val jsonRequest = JsonObjectRequest(Request.Method.GET,"https://www.googleapis.com/books/v1/volumes?q=$text&startIndex=$page", null,
            { jsonObject ->
                try {
                    ...
                    bookAdapter = BooksAdapter(books.toTypedArray(), { book -> bookOnclick() }, this@Favoris_ActivityActivity)
                    recyclerView!!.layoutManager  = LinearLayoutManager(this@Favoris_ActivityActivity)
